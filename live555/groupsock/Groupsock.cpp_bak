...
OutputSocket::OutputSocket(UsageEnvironment& env) :
	Socket(env, 0 /* let kernel choose port */), fSourcePort(0),
			fLastSentTTL(0) {
}

OutputSocket::OutputSocket(UsageEnvironment& env, Port port) :
	Socket(env, port), fSourcePort(0), fLastSentTTL(0) {
}
...
Boolean OutputSocket::write(netAddressBits address, Port port, u_int8_t ttl,
	...
	if (!writeSocket(env(), socketNum(), destAddr, port, ttl, buffer,
			bufferSize))
		return False;
...
destRecord::destRecord(struct in_addr const& addr, Port const& port,
		u_int8_t ttl, destRecord* next) :
...
/*
 * 类Groupsock：这个是存放在单独的库Groupsock中。它封装了socket操作，增加了多播放支持和一对
 * 多单播的功能。但我只看到它对UDP的支持，好像不支持TCP。它管理着一个本地socket和多个目的地址，
 * 因为是UDP，所以只需要知道对方的地址和端口号即可发送数据。Groupsock的构造函数有一个参数是
 * struct in_addr const& groupAddr,在构造函数中首先会调用父类构造函数创建socket对象，然后
 * 判断这个地址，若是多播地址，则加入多播组。Groupsock的两个成员变量destRecord* fDests和
 * DirectNetInterfaceSet fMembers都表示目的地址集合，但我始终看不出DirectNetInterfaceSet
 * fMembers有什么用，且DirectNetInterfaceSet是一个没有被继承的虚类，看起来fMembers没有什么
 * 用。仅fDesk也够用了，在addDestination()和removeDestination()函数中就是操作fDesk，添加
 * 或删除目的地址
 */
///////// Groupsock //////////
...
Groupsock::Groupsock(UsageEnvironment& env, struct in_addr const& groupAddr,
	...
	OutputSocket(env, port), deleteIfNoMembers(False), isSlave(False),
			fIncomingGroupEId(groupAddr, port.num(), ttl), fDests(NULL), fTTL(
					ttl) {
...
Groupsock::Groupsock(UsageEnvironment& env, struct in_addr const& groupAddr,
	...
	OutputSocket(env, port), deleteIfNoMembers(False), isSlave(False),
			fIncomingGroupEId(groupAddr, sourceFilterAddr, port.num()), fDests(
					NULL), fTTL(255) {
	addDestination(groupAddr, port);
...
void Groupsock::changeDestinationParameters(struct in_addr const& newDestAddr,
	 ...
	 * 都会把它插入到最前面，难道这个函数仅改变最后一个添加的目的地址？）
	 */
	struct in_addr destAddr = fDests->fGroupEId.groupAddress();
	if (newDestAddr.s_addr != 0) {
		if (newDestAddr.s_addr != destAddr.s_addr && IsMulticastAddress(
				newDestAddr.s_addr)) {
	...
	/*
	 * 目的地址的所有信息都在fGroupId中，所以改变成员的fGroupEId。
	 */
...
void Groupsock::addDestination(struct in_addr const& addr, Port const& port) {
	...
	for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
		if (addr.s_addr == dests->fGroupEId.groupAddress().s_addr && port.num()
				== dests->fPort.num()) {
			return;
		}
	}
...
void Groupsock::removeDestination(struct in_addr const& addr, Port const& port) {
	for (destRecord** destsPtr = &fDests; *destsPtr != NULL; destsPtr
			= &((*destsPtr)->fNext)) {
...
Boolean Groupsock::output(UsageEnvironment& env, u_int8_t ttlToSend,
	...
	do {
		...
		for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
			if (!write(dests->fGroupEId.groupAddress().s_addr, dests->fPort,
		...
		if (!members().IsEmpty()) {
			numMembers = outputToAllMembersExcept(interfaceNotToFwdBackTo,
					ttlToSend, buffer, bufferSize, ourIPAddress(env));
			if (numMembers < 0)
				break;
		}
...
Boolean Groupsock::handleRead(unsigned char* buffer, unsigned bufferMaxSize,
	...
	if (numBytes < 0) {
		if (DebugLevel >= 0) { // this is a fatal error
			env().setResultMsg("Groupsock read failed: ", env().getResultMsg());
		}
		return False;
	}

	// If we're a SSM group, make sure the source address matches:
	if (isSSM() && fromAddress.sin_addr.s_addr != sourceFilterAddress().s_addr) {
		return True;
	}
	...
	if (!wasLoopedBackFromUs(env(), fromAddress)) {
		...
		numMembers = outputToAllMembersExcept(NULL, ttl(), buffer, bytesRead,
...
int Groupsock::outputToAllMembersExcept(DirectedNetInterface* exceptInterface,
	...
	while ((interf = iter.next()) != NULL) {
		...
		if (numMembers == 0) {
			...
			unsigned trailerSize = TunnelEncapsulationTrailerSize
					+ trailerOffset;
...
UsageEnvironment& operator<<(UsageEnvironment& s, const Groupsock& g) {
	UsageEnvironment& s1 = s << timestampString() << " Groupsock("
			<< g.socketNum() << ": " << AddressString(g.groupAddress()).val()
			<< ", " << g.port() << ", ";
	if (g.isSSM()) {
