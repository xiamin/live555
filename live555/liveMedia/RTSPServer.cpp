 ...
 * fServerMediaSessions为RTSPServer的成员，是一个Hash表，定义为HashTable* fServerMediaSessions
 */
...
void RTSPServer::closeAllClientSessionsForServerMediaSession(
    ServerMediaSession* serverMediaSession)
{
    if (serverMediaSession == NULL)
        return;
...
void RTSPServer::deleteServerMediaSession(
    ServerMediaSession* serverMediaSession)
{
    if (serverMediaSession == NULL)
        return;
...
{
    RegisterRequestRecord* registerRequest = new RegisterRequestRecord(*this,
            serverMediaSession, responseHandler);
    ...
    do
    {
        ...
        {
            envir() << "Failed to find network address for \""
                    << remoteClientNameOrAddress << "\"";
            break;
        }
        netAddressBits remoteAddress =
...
#endif
        if (connect(sock, (struct sockaddr*) &remoteSockaddr,
                    sizeof remoteSockaddr) != 0)
        {
            ...
            if (err == EINPROGRESS || err == EWOULDBLOCK)
            {
                ...
                envir().taskScheduler(). setBackgroundHandling(
                    ...
                    (TaskScheduler::BackgroundHandlerProc*) &RegisterRequestRecord::connectionHandler,
                    registerRequest);
                return sock;
            }
...
int RTSPServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort)
{
    ...
    do
    {
        ...
        ourSocket = setupStreamSocket(env, ourPort); //建立流式套接字
        if (ourSocket < 0)
            break;
        ...
        if (listen(ourSocket, LISTEN_BACKLOG_SIZE) < 0)   //LISTEN_BACKLOG:请求队列中运寻的最大请求数，大多数系统缺省值为20
        {
...
/*
 * 要侦听客户端的连接，就需要利用任务调度机制了，所以需添加一个socket handler
 */
RTSPServer::RTSPServer(UsageEnvironment& env, int ourSocket, Port ourPort,
                       UserAuthenticationDatabase* authDatabase,
                       unsigned reclamationTestSeconds) :
    Medium(env), fRTSPServerPort(ourPort), fRTSPServerSocket(ourSocket),
    ...
    fAuthDB(authDatabase), fReclamationTestSeconds(
        reclamationTestSeconds)
{
...
RTSPServer::~RTSPServer()
{
    ...
    while ((connection
            = (RTSPServer::RTSPClientConnection*) fClientConnections->getFirst())
            != NULL)
    {
        delete connection;
    }
    ...
    RTSPServer::RTSPClientSession* clientSession;
    while ((clientSession
            = (RTSPServer::RTSPClientSession*) fClientSessions->getFirst())
            != NULL)
    {
        delete clientSession;
    }
    ...
    while ((registerRequest
            = (RTSPServer::RegisterRequestRecord*) fPendingRegisterRequests->getFirst())
            != NULL)
    {
...
/*
 * 当受到客户的连接的时需保存下代表客户端的新的socket，以后用这个socket与这个客户通讯
 * 每个客户将来会对应一个rtp会话，而且各客户的rtsp请求只控制自己的rtp会话，那么最好建立
 * 一个会话类，代表各客户的rtsp会话。于是类RTSPServer::RTSPClientSession产生，它保
 * 存的代表客户的socket。下面为RTSPClientSession的创建过程。
 */
...
int RTSPServer::continueRegisterStream(RegisterRequestRecord* registerRequest)
{
    ...
    int& sock = registerRequest->socketNum(); // alias
    char const* const formatStr = "REGISTER %s RTSP/1.0\r\n"
                                  "CSeq: 1\r\n\r\n";
    ...
    envir().taskScheduler(). setBackgroundHandling(
        ...
        (TaskScheduler::BackgroundHandlerProc*) &RegisterRequestRecord::incomingResponseHandler,
        registerRequest);
...
RTSPServer::RTSPClientConnection::RTSPClientConnection(RTSPServer& ourServer,
    ...
    fOurServer(ourServer), fIsActive(True), fClientInputSocket(clientSocket),
...
RTSPServer::RTSPClientConnection::ParamsForREGISTER::ParamsForREGISTER(
    ...
    char const* urlSuffix, Boolean registerRemote) :
    fOurConnection(ourConnection), fURL(strDup(url)), fURLSuffix(strDup(
                urlSuffix)), fRegisterRemote(registerRemote)
{
}
...
/*
 * 以下代码RTSPClientSession响应DESCRIBE请求的过程：
 */
/*
 * DESCRIBE函数在lookupServerMediaSession的过程中，首先根据名字实例化一个ServerMediaSession
 * 并将其加入到哈希表中，另外在创建SMS过程中，还将创建对应的子类H264VideoFileServerMediaSubsession::createNew
 */
    ...
    char const* fullRequestStr)
{
    ...
    do
    {
        ...
        /*
         * 根据流的名字查找ServerMediaSession,如果找不到，会创建一个。每个ServerMediasession中至少包含一个
         *
         * 一个ServerMediaSession对应一个媒体，可以认为是Server上的一个文件，或一个实时获取设备。其包含的每个
         * ServerMediaSubSession代表媒体中的一个Track。所以一个ServerMediaSession对应一个媒体，如果客户请求
         * 的媒体名相同，就使用已存在的ServerMediaSession，如果不同，就创建一个新的。一个流对应一个StreamState,
         * StreamState与ServerMediaSession相关，但代表的是动态的，而ServerMediaSession代表静态的。
         */
        ...
        //获取SDP字符串，在函数内依次获取每个ServerMediaSession的字符串然后连接起来
                          ...
                          unsigned sourceLen, char* resultStr, unsigned resultMaxSize)
{
    ...
    {
        if (strncmp(&source[i], headerName, headerNameLen) == 0 && source[i
                + headerNameLen] == ':')
        {
            // We found the header.  Skip over any whitespace, then copy the rest of the line to "resultStr":
            for (i += headerNameLen + 1; i < (int) sourceLen && (source[i]
                    == ' ' || source[i] == '\t'); ++i)
            {
            }
...
void RTSPServer::RTSPClientConnection::handleCmd_bad()
{
    ...
    snprintf((char*) fResponseBuffer, sizeof fResponseBuffer,
             ...
             fOurServer.allowedCommandNames());
}
    ...
    char const* sessionCookie)
{
    ...
    {
        fOurServer.fClientConnectionsForHTTPTunneling = HashTable::create(
                    STRING_HASH_KEYS);
    }
    delete[] fOurSessionCookie;
    fOurSessionCookie = strDup(sessionCookie);
    fOurServer.fClientConnectionsForHTTPTunneling->Add(sessionCookie,
            (void*) this);
#ifdef DEBUG
    ...
    unsigned extraDataSize)
{
    ...
    * prevClientConnection =
        (RTSPServer::RTSPClientConnection*) (fOurServer.fClientConnectionsForHTTPTunneling->Lookup(
                    sessionCookie));
...
void RTSPServer::RTSPClientConnection::incomingRequestHandler(void* instance,
        int /*mask*/)
{
...
void RTSPServer::RTSPClientConnection::incomingRequestHandler1()
{
    ...
    int bytesRead = readSocket(envir(), fClientInputSocket,
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead)
{
    ...
    do
    {
        ...
        if (newBytesRead < 0 || (unsigned) newBytesRead
        ...
        if (fClientOutputSocket != fClientInputSocket)
        {
            ...
            if (fBase64RemainderCount > 0)
                break; // because we know that we have more input bytes still to receive
        }
        ...
        if (parseSucceeded)
        {
            ...
            if (strcmp(cmdName, "OPTIONS") == 0)
            {
                ...
                {
                    clientSession
                        = (RTSPServer::RTSPClientSession*) (fOurServer.fClientSessions->Lookup(
                                sessionIdStr));
            ...
            else if (strcmp(cmdName, "TEARDOWN") == 0 || strcmp(cmdName,
                     "PLAY") == 0 || strcmp(cmdName, "PAUSE") == 0 || strcmp(
                         cmdName, "GET_PARAMETER") == 0 || strcmp(cmdName,
                                 "SET_PARAMETER") == 0)
            {
                RTSPServer::RTSPClientSession
                * clientSession =
                    sessionIdStr[0] == '\0' ? NULL
                    : (RTSPServer::RTSPClientSession*) (fOurServer.fClientSessions->Lookup(
                            sessionIdStr));
                if (clientSession == NULL)
                {
            ...
            else if (strcmp(cmdName, "REGISTER") == 0 || strcmp(cmdName,
                     "REGISTER_REMOTE") == 0)
            {
...
#endif
        send(fClientOutputSocket, (char const*) fResponseBuffer, strlen(
                 (char*) fResponseBuffer), 0);

        if (clientSession != NULL && clientSession->fStreamAfterSETUP
        ...
        char const* urlSuffix, char const* fullRequestStr)
{
    ...
    if (!fOurServer.specialClientAccessCheck(fClientInputSocket, fClientAddr,
            urlSuffix))
    {
    ...
    if (success)
    {
        ...
        if (!fOurServer.specialClientUserAccessCheck(fClientInputSocket,
        ...
        u_int32_t sessionId, char const* contentStr)
{
    ...
    snprintf((char*) fResponseBuffer, sizeof fResponseBuffer, "RTSP/1.0 %s\r\n"
             ...
             "Session: %08X\r\n"
             "Content-Length: %d\r\n\r\n"
             "%s", responseStr, fCurrentCSeq, dateHeader(), sessionId, contentLen,
             contentStr);
}

void RTSPServer::RTSPClientConnection::changeClientInputSocket(
    int newSocketNum, unsigned char const* extraData,
    unsigned extraDataSize)
{
    envir().taskScheduler().disableBackgroundHandling(fClientInputSocket);
...
void RTSPServer::RTSPClientConnection::continueHandlingREGISTER1(
    ParamsForREGISTER* params)
{
    int socketNumToBackEndServer = params->fRegisterRemote ? -1
                                   : fClientOutputSocket;
...
/*
 * RTSPClientSession要提供什么功能呢？可以想象：需要监听客户端的rtsp请求并回应它，需要在
 * DESCRIBE请求中返回所请求的流的信息，需要在SETUP请求中建立起RTP会话，需要在TEARDOWN
 * 请求中关闭RTP会话，等等
 *
...
RTSPServer::RTSPClientSession::RTSPClientSession(RTSPServer& ourServer,
    ...
    fOurServer(ourServer), fOurSessionId(sessionId), fOurServerMediaSession(
        NULL), fIsMulticast(False), fStreamAfterSETUP(False),
    fTCPStreamIdCount(0), fLivenessCheckTask(NULL),
...
/*
 * RTP的建立：
 * RTP的建立过程无非是这样的：client告诉server自己的rtp/rtcp端口号，server建立自己
 * 的rtp/rtcp socket,然后在受到PLAY请求时向客户端发数据。看起来非常简单。
    ...
    char const* fullRequestStr)
{
    ...
    do
    {
        ...
        parseTransportHeader(fullRequestStr, streamingMode,
                             streamingModeString, clientsDestinationAddressStr,
                             clientsDestinationTTL, clientRTPPortNum, clientRTCPPortNum,
        ...
        subsession->getStreamParameters(fOurSessionId,
                                        ...
                                        rtcpChannelId, destinationAddress, destinationTTL,
        ...
        if (fIsMulticast)
        {
            ...
            case RTP_UDP:
                snprintf((char*) ourClientConnection->fResponseBuffer,
                         ...
                         destAddrStr.val(), sourceAddrStr.val(), ntohs(
                             serverRTPPort.num()), ntohs(
                             serverRTCPPort.num()), destinationTTL,
                         fOurSessionId);
                break;
            case RTP_TCP:
        ...
        else
        {
            ...
            case RTP_UDP:
            {
                snprintf(
                    ...
                    ourClientConnection->fCurrentCSeq, dateHeader(),
                    destAddrStr.val(), sourceAddrStr.val(), ntohs(
                        clientRTPPort.num()), ntohs(
                        clientRTCPPort.num()), ntohs(
                        serverRTPPort.num()), ntohs(
                        serverRTCPPort.num()), fOurSessionId);
                break;
            }
            case RTP_TCP:
            {
            ...
            case RAW_UDP:
            {
                snprintf(
                    ...
                    ourClientConnection->fCurrentCSeq, dateHeader(),
                    streamingModeString, destAddrStr.val(),
                    sourceAddrStr.val(), ntohs(clientRTPPort.num()), ntohs(
                        serverRTPPort.num()), fOurSessionId);
                break;
            }
            }
        }
    ...
    char const* fullRequestStr)
{
    ...
    else if (urlSuffix[0] != '\0' && strcmp(
                 fOurServerMediaSession->streamName(), urlPreSuffix) == 0)
    {
    ...
    ServerMediaSubsession* subsession, char const* fullRequestStr)
{
    ...
    /*
      * 分析了"Scale" 头部
      * Scale 头部，指示了播放的速率，scale = 1为正常播放，大于1快进
      * 小于0则表示快退
      */
    ...
    //测试Scale 的值能否满足，这期间可能会改变scale的值
    ...
    scaleHeader = strDup(buf);	//复制

    //分析range 头部
    //range头部表示要播放的时间范围，如Range:npt = 0.000~,从0时刻开始播放到结束
    //不含range首部域的PLAY，请求也是合法的。他从媒体开头开始播放，直到媒体被暂停
    ...
    if (sawRangeHeader && absStart == NULL/*not seeking by 'absolute' time*/)
    {
        ...
        if (rangeEnd < 0.0)
            ...
            rangeEnd = duration;
        if ((scale > 0.0 && rangeStart > rangeEnd && rangeEnd > 0.0) || (scale
                < 0.0 && rangeStart < rangeEnd))
        {
...
RTSPServer::ServerMediaSessionIterator::ServerMediaSessionIterator(
    RTSPServer& server) :
    fOurIterator((server.fServerMediaSessions == NULL) ? NULL
...
UserAuthenticationDatabase::UserAuthenticationDatabase(char const* realm,
    ...
    fTable(HashTable::create(STRING_HASH_KEYS)), fRealm(strDup(
                realm == NULL ? "LIVE555 Streaming Media" : realm)),
...
RTSPServer::RegisterRequestRecord::RegisterRequestRecord(RTSPServer& ourServer,
        ServerMediaSession* serverMediaSession,
        responseHandlerForREGISTER* responseHandler) :
    fSocketNum(-1), fOurServer(ourServer), fServerMediaSession(
        serverMediaSession), fHandler(responseHandler)
{
...
void RTSPServer::RegisterRequestRecord::connectionHandler1()
{
    ...
    if (getsockopt(fSocketNum, SOL_SOCKET, SO_ERROR, (char*) &err, &len) < 0
            || err != 0)
    {
...
void RTSPServer::RegisterRequestRecord::incomingResponseHandler(void* instance,
        int /*mask*/)
{
    RegisterRequestRecord* registerRequest = (RegisterRequestRecord*) instance;
...
void RTSPServer::RegisterRequestRecord::incomingResponseHandler1()
{
    ...
    int bytesRead = readSocket(envir(), fSocketNum, responseBuffer,
                               REGISTER_RESPONSE_BUFFER_SIZE, dummy);
    if (bytesRead > 0 && bytesRead < REGISTER_RESPONSE_BUFFER_SIZE)
    {
                   ...
                   resultMsg) == 2)
        {
            ...
            if (resultCode == 200)
            {
                ...
                int sock = fSocketNum;
                fSocketNum = -1; // so that the socket doesn't get closed when we delete ourself below
                (void) fOurServer.createNewClientConnection(sock,
                        fRemoteAddress);
            }
        }
    }
...
void RTSPServer::RegisterRequestRecord::callResponseHandler(int resultCode,
...
char const* RTSPServerWithREGISTERProxying::allowedCommandNames()
{
    if (fAllowedCommandNames == NULL)
    {
        ...
        fAllowedCommandNames = new char[strlen(baseAllowedCommandNames)
...
void RTSPServerWithREGISTERProxying::implementCmd_REGISTER(char const* url,
        char const* urlSuffix, int socketToRemoteServer)
{
    ...
    addServerMediaSession(
        ProxyServerMediaSession::createNew(envir(), this, url, urlSuffix,
