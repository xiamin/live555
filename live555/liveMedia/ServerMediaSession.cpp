...
Boolean ServerMediaSession::lookupByName(UsageEnvironment& env,
		char const* mediumName, ServerMediaSession*& resultSession) {
...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
	...
	Medium(env), fIsSSM(isSSM), fSubsessionsHead(NULL), fSubsessionsTail(NULL),
			fSubsessionCounter(0), fReferenceCount(0), fDeleteWhenUnreferenced(
					False) {
	fStreamName = strDup(streamName == NULL ? "" : streamName);

	char* libNamePlusVersionStr = NULL; // by default
	if (info == NULL || description == NULL) {
		libNamePlusVersionStr = new char[strlen(libNameStr) + strlen(
				libVersionStr) + 1];
...
/*
 * live555支持的复合容器类型只有*.mpg,*.mkv, webm,可以看出程序中为容器中的每一个流建立一个subsession
 * 然后通过ServerMediaSession::addSubsession函数将subsession加入到ServerMediaSession
 *
 ...
 *
 * 继承关系：H264VideoFileServerMediaSubsession->FileServerMediaSubsession
...
float ServerMediaSession::duration() const {
	...
	for (ServerMediaSubsession* subsession = fSubsessionsHead; subsession
			!= NULL; subsession = subsession->fNext) {
...
char* ServerMediaSession::generateSDPDescription() {
	...
	if (fIsSSM) {
		...
		unsigned const sourceFilterFmtSize = strlen(sourceFilterFmt)
				+ ipAddressStrSize + 1;
	...
	do {
		...
		sdpLength += strlen(sdpPrefixFmt) + 20 + 6 + 20 + ipAddressStrSize
				...
				+ strlen(libNameStr) + strlen(libVersionStr) + strlen(
				sourceFilterLine) + strlen(rangeLine) + strlen(
				fDescriptionSDPString) + strlen(fInfoSDPString) + strlen(
				fMiscSDPLines);
		...
		sprintf(sdp, sdpPrefixFmt, fCreationTime.tv_sec, fCreationTime.tv_usec, // o= <session id>
...
ServerMediaSubsession::ServerMediaSubsession(UsageEnvironment& env) :
	Medium(env), fParentSession(NULL), fServerAddressForSDP(0), fPortNumForSDP(
			0), fNext(NULL), fTrackNumber(0), fTrackId(NULL) {
}
