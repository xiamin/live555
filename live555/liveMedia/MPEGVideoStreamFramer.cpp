...
/*
 *  这里的MPEGVideoStreamFramer::fParser,是一个MPEGVideoStreamParser类型指针，作为语法分析器
 */
...
/*
 *  函数中首先调用了MPEGVideoStreamParser::parse函数，将一个完整的Frame分析出来，并copy到
 *  了fTo(fTo就是OutPacketBuffer中的缓冲去)中，这其中肯定也实现了从文件读取数据的过程，这里
 *  的fNumTruncatedBytes变量需要注意，fNumTruncatedBytes>0的话，表明Frame的实际长度大于fTo
 *  的最大长度，这将导致数据丢失，这是就要考虑增加缓冲区的长度了，成功获取一个Frame后，将
 *  调用afterGetting函数处理后续工作。
 */
