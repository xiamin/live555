...
unsigned H264VideoStreamParser::parse()
{
    ...
    {
        /*
         * 首先找到起始符号，并跳过。文件流的最开始必需以0x00000001开始，但后续的NALU允许以0x000001(3 byte)作为分割
         */
        ...
        usingSource()->setPresentationTime();	//设置当前时间
#ifdef DEBUG
		 ...
		 *	我们需要根据这个信息去指明何时该增加"fPresentationTime"(RTP打包时也需要根据这个信息，决定是否设置"M"位).
		 */
        ...
        else
        {
            ...
            if (isVCL)
            {
                ...
                {
					//下一个NALU不是VCL的，当前"access unit"结束了
                ...
                {
					//下一个NALU也是VCL的，还需要检查一下他们是不是属于同一个"access unit"
