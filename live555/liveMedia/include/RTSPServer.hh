...
public:
	...
	void closeAllClientSessionsForServerMediaSession(
	...
	typedef void ( responseHandlerForREGISTER)(RTSPServer* rtspServer,
			int socketNum, int resultCode, char* resultString);
...
protected:
	...
	virtual void implementCmd_REGISTER(char const* url, char const* urlSuffix,
			int socketToRemoteServer); // ditto
...
public:
	...
	public:
		RTSPClientConnection(RTSPServer& ourServer, int clientSocket,
		...
		public:
			ParamsForREGISTER(RTSPClientConnection* ourConnection,
					char const* url, char const* urlSuffix,
	...
	protected:
		...
		virtual Boolean parseHTTPRequestString(char* resultCmdName,
				unsigned resultCmdNameMaxSize, char* urlSuffix,
	...
	protected:
		...
		// Shortcuts for setting up a RTSP response (prior to sending it):
		void setRTSPResponse(RTSPClientConnection* ourClientConnection,
...
public:
	...
	public:
		RegisterRequestRecord(RTSPServer& ourServer,
				ServerMediaSession* serverMediaSession,
				responseHandlerForREGISTER* responseHandler);
...
public:
	static RTSPServerWithREGISTERProxying* createNew(UsageEnvironment& env,
			Port ourPort = 554,
			UserAuthenticationDatabase* authDatabase = NULL,
			unsigned reclamationTestSeconds = 65);

protected:
	RTSPServerWithREGISTERProxying(UsageEnvironment& env, int ourSocket,
